...
if __name__ == "__main__":
    # datasets = get_datasets(dir='./sample')
    # print(datasets)

    graphs = []
    # for dataset in datasets:
    #     dataset.set_dot_file_path(generate_dot_by_radare2(dataset.bin_file_path, dataset.sample_name))
    #     if not os.path.exists(dataset.dot_file_path) or os.path.getsize(dataset.dot_file_path) == 0:
    ...
    # 手動で作ったものを埋め込む
    samples = ['out/sample/hello0.dot','out/sample/hello1.dot', 'out/sample/hello2.dot', 'out/sample/hello3.dot']
    for sample in samples:
        graphs.append(Graph(sample, convert_dot_to_networkx(sample)))
